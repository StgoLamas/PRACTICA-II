...
UB = 100 # upper bound for the objective function
b = 0.975 # discount factor used by Buongiorno
α = 0.5 # risk aversion parameter
...
#SDDP.train(
#    model;
#    stopping_rules = [SDDP.BoundStalling(10, 1e-3)],
#)
...
simulations = SDDP.simulate(model, n_simulations, [:x, :c])
objective_values = [
...
Plots.plot(
    SDDP.publication_plot(simulations, title = "x[1]") do data
        return data[:x][1].in # area occupied by trees in  age class 1
    end,
    SDDP.publication_plot(simulations, title = "x[2]") do data
        return data[:x][2].in # area occupied by trees in  age class 2
    end,
    SDDP.publication_plot(simulations, title = "x[3]") do data
        return data[:x][3].in # area occupied by trees in  age class 3
    end,
    SDDP.publication_plot(simulations, title = "x[4]") do data
        return data[:x][4].in # area occupied by trees in  age class 4
    end,
    SDDP.publication_plot(simulations, title = "x[5]") do data
        return data[:x][5].in # area occupied by trees in  age class 5
    end,
    xlabel = "Stage",
    ylims = (0, 1),
    layout = (2, 3),
...
Plots.plot(
    SDDP.publication_plot(simulations, title = "c[1]") do data
        return data[:c][1] # area harvested of trees in age class 1
    end,
    SDDP.publication_plot(simulations, title = "c[2]") do data
        return data[:c][2] # area harvested of trees in age class 2
    end,
    SDDP.publication_plot(simulations, title = "c[3]") do data
        return data[:c][3] # area harvested of trees in age class 3
    end,
    SDDP.publication_plot(simulations, title = "c[4]") do data
        return data[:c][4] # area harvested of trees in age class 4
    end,
    SDDP.publication_plot(simulations, title = "c[5]") do data
        return data[:c][5] # area harvested of trees in age class 5
    end,
    xlabel = "Stage",
    ylims = (0, 1),
    layout = (2, 3),
)
Plots.savefig("ForestHarvested.pdf")